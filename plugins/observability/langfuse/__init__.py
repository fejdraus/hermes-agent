...
def _mask_data_uris(*, data: Any, **_kwargs: Any) -> Any:
    """Убрать base64 data URI из полезной нагрузки до того, как её увидит SDK.

    `_truncate_text` вычищает те, что проходят через наши сериализаторы, но SDK
    обходит нагрузку сам и любую строку `data:*;base64,` принимает за медиа для
    декодирования. Всё, до чего мы не дотянулись — вложенное поле за пределом
    глубины, значение, которое мы не сериализовали, — доходит до его декодера, и
    обрезанный URI печатает «Error parsing base64 data URI» с полным стеком: за
    десять дней таких трейсбеков набралось 642 штуки, и настоящие ошибки в логе
    в них тонули.

    Наблюдаемости нужна форма нагрузки, а не пиксели, поэтому URI заменяется
    описанием: шум исчезает, сигнал остаётся.
    """
