...
def _try_inline_shell(script_path: str) -> tuple[Optional[bool], str]:
    """Выполнить значение как shell-однострочник, если это не имя файла.

    Контракт «script — это имя файла в ~/.hermes/scripts/» соблюдают не все:
    kilocode-auto-router и перенесённые из nanobot агенты передают готовую
    команду. Отказ здесь означал бы молча не выполненное задание, поэтому
    такая строка исполняется через bash. Признак — пробел, метасимвол или
    начало известной команды: имя файла ничего этого не содержит.

    Возвращает ``(None, "")``, когда значение похоже на имя файла и его надо
    отдать обычному резолверу.
    """
    if chr(0) in str(script_path):
        return False, f"Blocked: script path contains a NUL byte: {script_path!r}"
    text = str(script_path).strip()
    looks_inline = (
        " " in text
        or any(text.startswith(w) for w in _SHELL_STARTS)
        or any(c in text for c in _SHELL_METACHARS)
    )
    if not looks_inline:
        return None, ""
    ...
    try:
        ...
        result = subprocess.run(
            ...
            cwd=str(scripts_dir), env=build_subprocess_env(), **popen_kwargs,
        )
...
def _run_job_script(
    ...
    """
    inline_ok, inline_out = _try_inline_shell(script_path)
    if inline_ok is not None:
        return inline_ok, inline_out
